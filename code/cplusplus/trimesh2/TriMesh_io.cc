...
Can write: PLY (triangle mesh and range grid), OFF, OBJ, RAY, SM, STL, C++
*/
...
#define COND_READ_S(cond, where, len) do { if ((cond) && !is.read((where), (len))) return false; } while (0)
...
#define SWRITE(ptr, size, nmemb, stream) do { (stream).write((const char*)(ptr), (size)*(nmemb)); } while (0)
...
static bool read_stl(std::istream& is, TriMesh *mesh);
...
static bool write_stl(TriMesh *mesh, std::ostream& os);
...
// Read a TriMesh from a file.  Defined to use a helper function to make
// subclassing easier.
TriMesh *TriMesh::read_memory_stl(std::istream& is)
{
	TriMesh *mesh = new TriMesh();
...
static bool read_stl(std::istream& is, TriMesh *mesh)
{
	...
	char header[80];
	COND_READ_S(true, header, 80);

	int nfacets;
	COND_READ_S(true, (char*)&nfacets, 4);
	if (need_swap)
		swap_int(nfacets);
...
bool TriMesh::write(const char *filename)
{
	...
	enum { PLY_ASCII, PLY_BINARY_BE, PLY_BINARY_LE,
	       RAY, OBJ, OFF, SM, STL, CC } filetype;
...
// Write mesh to a file
bool TriMesh::write_memory_stl(std::ostream& stl)
{
	if (vertices.empty()) {
		eprintf("Empty mesh - nothing to write.\n");
		return false;
	...
	bool ok = write_stl(this, stl);

	if (!ok) {
		eprintf("Error writing stl in memory.\n");
		return false;
	}
...
static bool write_stl(TriMesh *mesh, std::ostream& os)
{
	...
	char header[80];
	memset(header, ' ', 80);
	SWRITE(header, 80, 1, os);

	int nfaces = mesh->faces.size();
	if (need_swap)
		swap_int(nfaces);
	SWRITE(&nfaces, 4, 1, os);
