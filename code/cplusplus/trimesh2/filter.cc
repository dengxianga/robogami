...
void apply_xform(TriMesh *mesh, const xform &xf, bool recalNormal)
{
	int nv = mesh->vertices.size();
...
#pragma omp parallel for
	for (int i = 0; i < nv; i++)
		mesh->vertices[i] = mesh->vertices[i] + transvec;
	if (mesh->bbox.valid) {
		mesh->bbox.min += transvec;
		mesh->bbox.max += transvec;
	}
	if (mesh->bsphere.valid) {
		mesh->bsphere.center += transvec;
	}
