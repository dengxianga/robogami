...
public:
	...
	{
		passes = int(std::log((float)xsize)/std::log(MAGIC_SCALE) + 0.5);
		passes = std::max(passes, int(std::log((float)ysize)/std::log(MAGIC_SCALE) + 0.5));
		passes = std::max(passes, int(std::log((float)zsize)/std::log(MAGIC_SCALE) + 0.5));
		float factor = 1.0;
