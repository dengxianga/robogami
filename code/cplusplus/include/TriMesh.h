...
public:
	...
	TriMesh * crunch()
	{
		bool had_tstrips = !this->tstrips.empty();

		TriMesh *out = new TriMesh;
		float voxelsize = 2.0f * this->feature_size();
		
		// perfrom the vertex collapse
		size_t nv = this->vertices.size();
		// in->flags stores which output vertex we mapped to
		this->flags.clear();
		this->flags.resize(nv);
		...
		for (size_t i = 0; i < nv; i++) {
			size_t &ind = hash[this->vertices[i]];
			if (ind >= nv) {
				// Failed lookup - add a new output vertex
				ind = out->vertices.size();
				out->vertices.push_back(this->vertices[i]);
				out->flags.push_back(1);
			} else {
				out->vertices[ind] += this->vertices[i];
				out->flags[ind]++;
			}
			this->flags[i] = ind;
		}
		...
		out->faces.reserve(std::min(nf, onv*3));
		for (size_t i = 0; i < nf; i++) {
			size_t ind0 = this->flags[this->faces[i][0]];
			size_t ind1 = this->flags[this->faces[i][1]];
			size_t ind2 = this->flags[this->faces[i][2]];
...
public:
	...
	static TriMesh *read_memory_stl(std::istream &istream);
	bool write_memory_stl(std::ostream& ostream);
