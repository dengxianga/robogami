...
namespace FBE_CSharpUI
{
    class ScaleGizmo
    {
        ...
        public ScaleGizmo(Viewport3D viewport)
        {
                    ...
                    if (whichAxis != 0)
                    {
                      ...
                      //  double dx = Math.Pow(2, (curPosition.X - lastPosition.X)/200);
                        ...
                        if(whichAxis==1){
                            ...
                            scaling = new ScaleTransform(Math.Pow(2,proj/200), 1);

                        }else if(whichAxis==2){
                            //scale along y
                            Vector offset = curPosition - lastPosition;
                            double proj = sY*offset/sY.Length;
                            scaling = new ScaleTransform(1, Math.Pow(2, proj / 200));
                        }
